...
st.markdown("""
Este planejador cria um plano tático completo de mídias pagas, com fluxo lógico de informações:
...
# ETAPA 3: Arquitetura de Canais
    ...
    with col1:
        if st.button("✅ Avançar para Estrutura do Plano", use_container_width=True):
            with st.spinner("Estruturando fases do plano..."):
...
# ETAPA 5: Detalhamento Tático
    ...
    if 'fases_detalhadas' not in st.session_state.narrativa_gerada:
        with st.spinner("Detalhando cada fase..."):
    ...
    with col1:
        if st.button("✅ Avançar para Cronograma", use_container_width=True):
            with st.spinner("Gerando cronograma de execução..."):
...
"""
            ...
            with st.spinner("Gerando recomendações executivas..."):
...
# ETAPA 7: Plano Final
    ...
    # Mostrar recomendações primeiro
...
st.markdown("---")
st.markdown("""
*Planejador Tático de Mídias Pagas - Baseado em dados do mercado brasileiro e frameworks acadêmicos.*
""")
